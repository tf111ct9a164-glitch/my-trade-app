...
export async function GET(request) {
  try {
    ...
    // 3. 静的な全銘柄リスト（CONSTITUENTS）に、DBから見つかった株価をJOINマージ
    const stocks = CONSTITUENTS.map((s) => {
      const dbInfo = priceMap.get(s.code);
      return {
        code: s.code,
        name: s.name,
        sector: s.sector,
        price: dbInfo ? dbInfo.price : null,
        updatedAt: dbInfo ? dbInfo.updatedAt : null
      };
    });
